...
let slideHandler = setInterval(() => {
    let index = Array.from(sliderImgs).findIndex((item) => item.classList.contains('active'))
...
// console.log(logoSlider.offsetWidth);

// logoSlider.onscroll = () => {
//     console.log(logoSlider.scrollLeft);
// }
...
const handleLogoSlider = setInterval(() => {
    if (logoSlider.offsetWidth - logoSlider.scrollLeft >= 5) {
        ...
        clearInterval(handleLogoSlider)
}, 10)
